...
function App() {
  ...
  const submitAuth = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setNotice({
        ...
        text:
          authMode === "register"
            ? "Account created. Welcome to your shelf."
            : "Welcome back.",
      });
      setPassword("");
    } catch (error) {
  ...
  const buy = async (book: Book) => {
    ...
    try {
      ...
      const checkout = new window.Razorpay({
        ...
        theme: { color: "#c45735" },
        modal: { ondismiss: () => setBusyBook(null) },
        handler: async (response: Record<string, string>) => {
          try {
            await verifyPayment(response).unwrap();
            setNotice({
              type: "success",
              text: "Payment verified. Your book is ready to download.",
            });
          } catch (error) {
            setNotice({ type: "error", text: (error as Error).message });
          } finally {
  ...
  const download = async (bookId: string, title: string) => {
    ...
    if (!response.ok) {
      setNotice({
        type: "error",
        text: "This book is not available for download.",
      });
      return;
    }
    ...
    return (
      <Box className="auth-page">
        <Box className="auth-panel">
          ...
          <Typography className="eyebrow">THE DIGITAL BOOKSHELF</Typography>
          ...
          <Typography className="lead">
            Sign in to explore the collection and keep your next chapter close.
          </Typography>
          {notice && <Alert severity={notice.type}>{notice.text}</Alert>}
          <Box component="form" onSubmit={submitAuth} className="auth-form">
            <TextField
              label="Email"
              type="email"
              value={email}
    ...
    <>
      <AppBar position="sticky" elevation={0} color="transparent">
        <Toolbar className="toolbar">
          ...
          <Box sx={{ flexGrow: 1 }} />
          <Button className="purchased-link" href="#purchased-books">
            Purchased Books
          </Button>
          <Typography className="user-email">{user.email}</Typography>
          <Button onClick={() => dispatch(clearSession())}>Log out</Button>
        </Toolbar>
      </AppBar>
      <main>
        <section className="intro">
          ...
          <Typography className="lead">
            A small, thoughtful collection of books for curious minds. Read
            deeply, return often.
          </Typography>
        </section>
        {notice && (
        ...
        <Container maxWidth="lg" className="catalog">
          ...
          >
            <Box>
              <Typography className="eyebrow">OUR COLLECTION</Typography>
              <Typography variant="h2">Find your next chapter</Typography>
            </Box>
            <Typography className="book-count">
              {books.length} titles
            </Typography>
          </Box>
              ...
              <Card className="book-card" key={book.id}>
                ...
                <CardActions>
                  ...
                  <Button
                    ...
                    onClick={() => buy(book)}
                  >
                    {busyBook === book.id ? "Opening..." : "Buy book"}
                  </Button>
                </CardActions>
              </Card>
            ))}
        ...
        <Container id="purchased-books" maxWidth="lg" className="orders">
          <Divider />
          <Typography className="eyebrow">YOUR SHELF</Typography>
          <Typography variant="h2">Purchased books</Typography>
          {orders.length === 0 ? (
            ...
            <Box className="order-list">
              {orders.map((order) => (
                <Box className="order-row" key={order.id}>
                  <Box>
                    <Typography className="book-title">
                      {order.book.title}
                    </Typography>
                    <Typography className="author">
                      {order.book.author} · ₹{order.amount}
                    </Typography>
                  </Box>
                  {order.status === "PAID" ? (
                    <Button
                      onClick={() => download(order.bookId, order.book.title)}
                    >
                      Download
                    </Button>
                  ) : (
                    <Typography className="pending">Payment pending</Typography>
                  )}
                </Box>
              ))}
            </Box>
